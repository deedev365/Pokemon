...
class Pokemon extends Model
{
    ...
    public function filterPokemons(): void
    {
        foreach($this->pokemonList as $index => $pokemon) {
            ...
            if($this->removeLegendaryPokemon($index, $pokemon['legendary'])) {
                continue;
            }
    ...
    /**
     * @param int    $index
     * @param string $legendary 
     */
    public function removeLegendaryPokemon(int $index, string $legendary): bool
    {
        $removeStatus = false;
        if($legendary === 'True') {
     ...
     * @param float $factor
     */
    public function modifyPokemonByType(int $index, string $type, string $action, string $field = '', float $factor = 0.0): void
    {
     ...
     * @param float $factor
     */
    public function modifyPokemonByLatter(int $index, string $latter, string $field, float $factor): void
    {
